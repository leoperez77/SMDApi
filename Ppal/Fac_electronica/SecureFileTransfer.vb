...
Public Class SecureFileTransfer
	...
	Public Function putFile(ByVal localFile As String, ByVal remotePath As String) As Boolean
		...
		Catch ex As System.Exception
			NoReloj()
			If ex.Message.Contains("No se puede establecer una conexión") Then
				Mensaje(Idi("El sitio FTP denegó el acceso, solicite el permiso con el administrador de la red") & ": [" & Me._hostname & "]")
			Else
	...
	Public Function getDirectoryList(ByVal remotePath As String) As ArrayList
		...
		Catch ex As System.Exception
			NoReloj()
			If ex.Message.Contains("No se puede establecer una conexión") Then
				Mensaje("El sitio FTP [" & Me._hostname & "] denegó el acceso, solicite el permiso con el administrador de la red")
			Else
